...
<div class="container mt-4 repair-form-page">
	...
	<form method="POST" action="{{ route('it-repair-tickets.store') }}" enctype="multipart/form-data">@csrf
		<div class="card repair-form"><div class="card-header"><i class="bi bi-life-preserver"></i>Form Tiket User <span>Permintaan Perbaikan IT</span></div><div class="card-body"><div class="row g-3">
			<div class="col-md-6">
				<label class="form-label">Peralatan IT</label>
				<div class="d-flex align-items-start" style="gap:.5rem;">
					<input id="equipmentCode" name="equipment_code" type="search" class="form-control form-control-sm" placeholder="Kode" style="width:110px;" autocomplete="off">
					<select id="equipmentSelect" name="equipment_id" class="form-select">
						<option value="">-- Pilih Peralatan --</option>
						@foreach($equipment as $item)
							<option value="{{ $item->id }}" data-search="{{ strtolower($item->name . ' ' . ($item->asset_tag ?? '') . ' ' . ($item->owner_name ?? '') . ' ' . ($item->assetLocation?->name ?? $item->getRawOriginal('location') ?? '')) }}" {{ (string) old('equipment_id', request('equipment_id')) === (string) $item->id ? 'selected' : '' }}>{{ $item->name }} - {{ $item->owner_name ?: 'PIC belum diisi' }} - {{ $item->assetLocation?->name ?: ($item->getRawOriginal('location') ?: 'Lokasi belum diisi') }}</option>
						@endforeach
					</select>
				</div>
				<small id="equipmentSearchHint" class="form-text">Cari menggunakan inisial atau nama peralatan dan PIC.</small>
			</div>
...
function filterEquipmentOptions() {
	const keyword = (equipmentCode?.value || '').trim().toLowerCase();
	let matches = 0;
	let lastMatch = null;
	equipmentOptions.forEach(option => {
		const hay = (option.dataset.search || option.text || '').toLowerCase();
		const match = !keyword || hay.includes(keyword);
		option.hidden = !match;
		option.disabled = !match;
		if (match) { matches += 1; lastMatch = option; }
	});
	equipmentSearchHint.textContent = (keyword || code) ? matches + ' peralatan ditemukan.' : 'Cari menggunakan inisial atau nama peralatan dan PIC.';
	// auto-select when exactly one match
	if (matches === 1 && lastMatch) {
		equipmentSelect.value = lastMatch.value;
	}
}
equipmentCode?.addEventListener('input', filterEquipmentOptions);
function isSoftware() {
