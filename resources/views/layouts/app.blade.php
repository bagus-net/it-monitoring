...
<body>
    ...
    <main class="app-main py-4">
        <div class="app-topbar">
            ...
            @auth
            <a href="{{ route('profile.show') }}" class="topbar-user text-decoration-none"><span class="topbar-profile-photo"><img src="{{ auth()->user()->profile_photo_path ? asset('storage/' . auth()->user()->profile_photo_path) : asset('images/default-avatar.svg') }}" alt="Foto profil {{ auth()->user()->name }}"></span><span><span class="topbar-user-name">{{ auth()->user()->name }}</span><span class="topbar-user-role">{{ auth()->user()->signature_title ?: (auth()->user()->department ?: 'Jabatan') }} | {{ auth()->user()->roleLabel() }}</span></span></a>
