...
@section('content')
    ...
    <div class="container mt-4 repair-page">
        ...
        <div class="row g-3 mb-3">
                ...
                <div class="col-md-3"><a class="repair-stat repair-in-progress"
                ...
                @if (!auth()->user()->isEmployee())
                    <div class="col-md-3"><div class="repair-stat approval"><span>Approval</span><strong>{{ $summary['approved'] }}</strong><small><a href="{{ route('it-repair-tickets.index', ['approval' => 'approved']) }}">sudah disetujui</a> · <a href="{{ route('it-repair-tickets.index', ['approval' => 'pending']) }}">{{ $summary['pending_approval'] }} belum</a></small></div></div>
                @endif
                <div class="col-md-3"><a class="repair-stat resolved"
        ...
        <div class="card repair-filter mb-3">
            ...
            <div class="card-body">
                <form method="GET" action="{{ route('it-repair-tickets.index') }}" class="row g-2 align-items-end">
                    ...
                    @if (!auth()->user()->isEmployee())
                        <div class="col-6 col-lg-2"><label class="form-label">Approval</label><select name="approval"
                                class="form-select">
                                <option value="">Semua</option>
                                <option value="approved" @selected($approval === 'approved')>Sudah disetujui</option>
                                <option value="pending" @selected($approval === 'pending')>Belum disetujui</option>
                            </select>
                        </div>
                    @endif
                        ...
                        <tbody>
                            @forelse($tickets as $ticket)
                                <tr>
                                    ...
                                    @if (!auth()->user()->isEmployee())
                                        <td><span class="approval-status {{ $ticket->approved_at ? 'approval-approved' : 'approval-pending' }}">
                                                <i class="bi {{ $ticket->approved_at ? 'bi-check-circle-fill' : 'bi-clock' }}"></i>
                                                {{ $ticket->approved_at ? 'Sudah disetujui' : 'Belum disetujui' }}
                                            </span></td>
                                    @endif
                                    ...
                                    <td colspan="{{ auth()->user()->isEmployee() ? 11 : 13 }}" class="text-center text-muted py-4">Belum ada tiket perbaikan IT.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
    ...
    <style>
        ...
        .repair-page .repair-stat{position:relative;min-height:116px;padding:19px 20px;border:1px solid #fbe5e9;border-top:0;border-radius:16px;background:#fff;box-shadow:0 7px 20px rgba(196,61,88,.06);overflow:hidden}.repair-page .repair-stat:after{content:'';position:absolute;right:-22px;bottom:-34px;width:90px;height:90px;border-radius:50%;background:currentColor;opacity:.06}.repair-page .repair-stat span{color:#78849a;font-size:.72rem;font-weight:700}.repair-page .repair-stat strong{display:block;margin-top:9px;color:#18243d;font-size:1.85rem;font-weight:800}.repair-page .repair-stat small{color:#8792a7;font-size:.68rem}.repair-page .repair-stat small a{color:inherit;text-decoration:none}.repair-page .repair-stat small a:hover{text-decoration:underline}.repair-page .repair-stat.open{color:#f3b54b}.repair-page .repair-stat.repair-in-progress{color:#7c5cfc}.repair-page .repair-stat.approval{color:#f59e0b}.repair-page .repair-stat.resolved{color:#14b8a6}
